...
import { bracket, resolve } from "./Helper";
import { util } from "../../../../../../source/Domain/Util";
...
export const data: Array<TestRecord> = [
	...
	},
	{
		...
		tests: [{ output: null }, { output: null, "foo.bar.baz": 100 }],
	},
	{
		operator: "$getField",
		query: { field: "foo.bar.baz", input: { "foo.bar.baz": 4 } },
		tests: [{ output: 4 }, { output: 4, "foo.bar.baz": 100 }],
	},
];
...
test(
	"Domain/Filter/Operator/Evaluation/Expression/Misc - $sampleRate",
	bracket(
		// before => after
		() => {
			const { rand } = util;
		...
		(t) => {
			[0.1, 0.2, 0.25, 0.33, 0.4, 0.5, 0.75, 1].forEach((rate) => {
				for (let length = 100; length < 1e6; length *= 10) {
					// phase=0 makes the run deterministic: no randomness, exact counts
					const compiled = Misc.$sampleRate(rate, undefined);
					const samples = Array.from({ length }, () => ({}));
					const result = samples.filter(compiled);

					if (rate < 1) {
						const expected = length * rate;
						const min = Math.floor(expected);
						const max = Math.ceil(expected);

						t.true(
							result.length >= min && result.length <= max,
							`$sampleRate ${rate} on ${length}: got ${result.length}, expected ${min}–${max}`,
						);
					} else {
						t.true(
