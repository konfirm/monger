...
function single<Q = Numeric, I = number, O = number>(
	fn: (i: I) => O,
): (q: Q, c: ExpressionCompiler) => Evaluator<O> {
...
function calculate<Q = Numeric, I = Nullable<number>, O = Nullable<number>>(
	fn: (value: I) => O,
	unresolved: O = <O>null,
	nan: O = <O>NaN,
): (query: Q, compiler: ExpressionCompiler) => Evaluator<O> {
	return single<Q, I, O>(
		(input: I): O =>
			isUnresolved(input)
				? unresolved
 ...
 */
export function $add(
	query: [Numeric | Date, Numeric | Date, ...Array<Numeric | Date>],
	compile: ExpressionCompiler,
): Evaluator<number | Date | null> {
	...
	return (input: any) => {
		...
		return dates.length ? new Date(dates[0].getTime() + sum) : sum;
	};
}
...
export function $multiply(
	query: [Numeric, Numeric, ...Array<Numeric>],
	compile: ExpressionCompiler,
): Evaluator<Nullable<number>> {
	const resolve = query.map(compile);

	return (input: any) =>
		resolve
			.map((f) => f(input))
			.reduce((c, v) => (isNumber(c) && isNumber(v) ? c * v : null)) as
			| number
			| null;
}
...
export function $sqrt(
	...
	return (input: any) => {
		...
		if (isNumber(number)) {
			if (number < 0) {
				throw new Error(
					`$sqrt requires a positive value, got ${number}`,
				);
			}
...
export function $subtract(
	query: [Date | Numeric, Date | Numeric],
	compile: ExpressionCompiler,
): Evaluator<Date | number | null> {
	const resolve = query.map(compile);
