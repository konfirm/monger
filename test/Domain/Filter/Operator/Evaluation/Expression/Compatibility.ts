...
function runTestCase(operator: string, testCase: any, t: any) {
	...
	if (error) {
		...
		t.throws(
			...
			new RegExp(error),
			`${operator}: ${description} - should throw error`,
		);
	} else {
		// Handle normal cases
		try {
			const resolvedQuery = resolveQuery(query[operator], context);
			console.dir({ [operator]: resolvedQuery });
			const compiled = expression({ [operator]: resolvedQuery });
			...
			expected.forEach((expectedDoc: any) => {
				...
				const outputField = Object.keys(expectedDoc).find(
					(k) => k !== "_id",
				);
				if (outputField) {
			...
			t.fail(
				`${operator}: ${description} - threw unexpected error: ${err.message}`,
			);
		}
		console.groupEnd();
	}
...
testModules.forEach((testModule) => {
	...
	test(`MongoDB Compatibility - ${operator}`, (t) => {
		...
		Object.entries(tests).forEach(
			([category, testCases]: [string, any]) => {
				testCases
					.filter(({ description }: any) =>
						/Operation 1[01]/.test(description),
					)
