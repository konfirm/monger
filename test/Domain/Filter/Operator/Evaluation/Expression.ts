...
type TestError = {
	error: RegExp | string | ((error: Error) => boolean);
	[key: string]: unknown;
};
...
export const data: Array<TestRecord> = [
	{
		operator: "$nope",
		query: 10,
		tests: [{ error: /Invalid expression/ }],
	},
	...arithmetic,
	...array,
	...boolean,
...
test("Domain/Filter/Operator/Evaluation/Expression - exports", (t) => {
	...
	expected.forEach((key) => {
		t.equal(
			typeof Expression[<keyof typeof Expression>key],
			"function",
		...
		tests.forEach(({ output, error, ...input }: any) => {
			if (error) {
				t.throws(
					...
					error,
					`${JSON.stringify(q)} on ${JSON.stringify(input)} throws ${error}`,
				);
			} else {
				const compiled = expression(q);
				t.deepEqual(
					compiled(input),
					output,
					`${JSON.stringify(q)} on ${JSON.stringify(input)} equals ${JSON.stringify(output)}`,
				);
			}
		});
	});
